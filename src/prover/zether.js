...
class ZetherProver {
    ...
    recursivePolynomials (list, accum, p, q) {
        ...
        this.recursivePolynomials(list, accum.mul(left), p, q);
        this.recursivePolynomials(list, accum.mul(right), p, q);
        p.push(pTop);
        q.push(qTop);
    }
    ...
    generateProof (statement, witness) {
        ...
        proof.A = this.params.commit(aL, aR, alpha);
        ...
        proof.S = this.params.commit(sL, sR, rho);
        ...
        proof.P = this.params.commitRows(p, r_P);
        proof.Q = this.params.commitRows(q, r_Q);
        proof.U = this.params.commitRows(u, r_U);
        proof.V = this.params.commitRows(v, r_V);
        proof.X = this.params.commitRows([p[0].hadamard(p[m])], r_X);
        proof.Y = this.params.commitRows([p[0].hadamard(q[m]).add(p[m].hadamard(q[0]))], r_Y);
        ...
        var poly_0 = [];
        var poly_1 = [];
        this.recursivePolynomials(poly_0, new Polynomial(), p.slice(0, m), q.slice(0, m));
        this.recursivePolynomials(poly_1, new Polynomial(), p.slice(m), q.slice(m));
        ...
        proof.CRnG = Array.from({ length: m }).map((_, i) => statement['CRn'].commit(poly_0[i]).add(this.params.getG().mul(pi[i])));
        ...
        proof.y_XG = Array.from({ length: m }).map((_, i) => this.params.getG().mul(sigma_X[i]));
        var dPow = new BN(1).toRed(bn128.q);
        for (var j = 0; j < N; j++) { // could turn this into a complicated reduce, but...
            var temp = this.params.getG().mul(witness['bTransfer'].redMul(dPow));
        ...
        proof.gG = Array.from({ length: m }).map((_, i) => this.params.getG().mul(sigma_0[i]));
        ...
        for (var i = 0; i < m; i++) {
            CRnR = CRnR.add(this.params.getG().mul(pi[i].redNeg().redMul(wPow)));
            ...
            gR = gR.add(this.params.getG().mul(sigma_0[i].redNeg().redMul(wPow)));
        ...
        proof.CPrime = this.params.getH().mul(gammaTransfer.redMul(wPow)).add(y_0R.mul(zetaTransfer));
        proof.DPrime = gR.mul(zetaTransfer);
        proof.CLnPrime = this.params.getH().mul(gammaDiff.redMul(wPow)).add(y_0R.mul(zetaDiff));
        ...
        var polyCommitment = new PolyCommitment(this.params, tPolyCoefficients, zs[0].redMul(gammaTransfer).redAdd(zs[1].redMul(gammaDiff)));
        ...
        var A_y = gR.mul(k_sk);
        var A_D = this.params.getG().mul(k_r); // gR........ no longer
        ...
        var A_C0 = this.params.getG().mul(k_vTransfer).add(DR.mul(k_sk));
        var A_CLn = this.params.getG().mul(k_vDiff).add(CRnR.mul(k_sk));
        var A_CPrime = this.params.getH().mul(k_nuTransfer).add(proof.DPrime.mul(k_sk));
